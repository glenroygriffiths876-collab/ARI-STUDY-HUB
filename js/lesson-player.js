...
function learnStages(c,u,s,lesson){
  ...
  stages.push({title:'Watch the idea being used',render:()=>{const d=make('div');(lesson.worked||lesson.workedExamples||[]).forEach((x,i)=>{const ex=make('article',{class:'worked'});ex.append(make('span',{class:'number-badge',text:String(i+1)}),para(x));d.append(ex);});return d;}});
  stages.push({title:'Connect it to Jamaica or everyday life',render:()=>make('div',{},[para((lesson.jamaica||lesson.jamaicaConnection||'')),make('p',{class:'caption',text:'The local example is here to make the idea concrete. The rule or concept still has to be explained accurately.'})])});
  stages.push({title:'Mistakes to avoid',render:()=>{const d=make('div');(lesson.mistakes||lesson.commonMistakes||[]).forEach(m=>{const x=make('article',{class:'mistake'});x.append(make('strong',{text:`Watch out: ${m.mistake}`}),para(`Fix it: ${m.correction}`));d.append(x);});return d;}});
  return stages;
}
function renderLearn(container,c,u,s){const lesson=LESSON_CONTENT[c.id]||buildLessonContent(c,u,s),p=progressFor(c),stages=learnStages(c,u,s,lesson);let idx=Math.min(p.learnIndex||0,stages.length-1);const stage=stages[idx],body=stage.render();const ccard=card(stage.title,body,{className:'learning-stage'});const tools=make('div',{class:'actions between'});tools.append(idx>0?button('← Previous',()=>{save(c,{learnIndex:idx-1});renderLesson(container,c.id);}):make('span'));
